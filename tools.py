...
def selectionDict(selref, ptmin, ptmax, nsteps):
    
    seldict = collections.OrderedDict()
    step = (ptmax - ptmin)/nsteps
    
    for i in xrange(nsteps+1):
       ptval = ptmin + i*step
       selstr = '{} && higgs_pt > {}'.format(selref,ptval)
       seldict[ptval] = selstr
    return seldict

#_____________________________________________________________________________________________________
def producePlots(selBase, proclist, groups, colors, variables, ptmax, nsteps, unc, name, lumi, version, run_full):
    ...
    seldict = selectionDict(selBase, 0., ptmax, nsteps)
